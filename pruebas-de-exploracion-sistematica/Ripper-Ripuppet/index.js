...
async function recursiveExploration(page, link, depth, parentState){
  ...
  if(depth > depthLevels) {
    console.log("Depth leve ls reached. Exploration stopped")
    return;
  } 
  ...
  if(!!body) {
    ...
    if(depth === 0){
      let elementsSignIn = []
      await getTextInputs(page, elementsSignIn);
      console.log(elementsSignIn)
      await interactWithObjects(elementsSignIn,page,currentState,link)
